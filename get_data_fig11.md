...
```{code-cell} ipython3
# manually remove problem models
fig11_models.remove("EC-Earth3-Veg")
fig11_models.remove("EC-Earth3-Veg-LR")
fig11_models.remove("HadGEM3-GC31-MM")
fig11_models.remove('TaiESM1')
```

```{code-cell} ipython3
# parse each model in the list 
for source in fig11_models:
    source_id = source
    ...
    for dim in ["height", "time_bounds", "depth", "depth_bounds", "lat_bnds", "lon_bnds", "time_bnds"]:
        try:
    ...
    table_id = 'day'
    %run CMIP6_lib.ipynb
    required_fields = ['mrsos', "pr", 'hfls', 'hfss']
    ...
    if save_data:
        ...
        full_dataset.to_netcdf(f"./data/{source_id}-{experiment_id}-fig11.nc", engine="netcdf4")
